...
public class ResourceCreditCardEngine extends ResourceEngine {
	...
	public List<JournalEntry> generateJournalEntries(YearMonth yearMonth, BalanceTracker balanceTracker) {
		...
		if (!monthlyInterestAmount.equals(BigDecimal.ZERO)) {
			journalEntries.add(JournalEntryFactory.create(getResource(), yearMonth, integerOf(monthlyInterestAmount), CashflowCategory.EXPENSE_INTEREST));
		}
	...
	private BigDecimal interestRate(final YearMonth yearMonth) {
		Optional<ResourceParamDateValue<?>> interestRateOpt = (Optional<ResourceParamDateValue<?>>) getResource().getResourceParamDateValue(ResourceParamNameEnum.CREDIT_CARD_INTEREST_RATE, yearMonth);
